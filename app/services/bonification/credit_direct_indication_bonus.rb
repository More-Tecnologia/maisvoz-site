  ...
  class CreditDirectIndicationBonus
    ...
    def credit_bonus
      FinancialEntry.new.tap do |bonus|
        bonus.user        = sponsor
        bonus.description = "Bônus de indicação direta da compra do usuário #{user.username}. Bônus de #{h.number_to_currency bonus_amount}"
        bonus.kind        = FinancialEntry.kinds[:direct_indication_bonus]
    ...
    def reverse_bonus
      FinancialEntry.new.tap do |bonus|
        bonus.user        = sponsor
        bonus.description = "[Estorno] Bônus de indicação direta da compra do usuário #{user.username}. Bônus de #{h.number_to_currency bonus_amount}"
